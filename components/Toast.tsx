...
const Toast: React.FC<Props> = ({ id, message, type = 'success', removeToast }) => {
    ...
    return (
        <div 
            className={`relative flex items-start w-full max-w-sm p-4 bg-surface/80 backdrop-blur-lg border border-border rounded-xl shadow-2xl overflow-hidden ${
        ...
        >
            ...
            <button
                onClick={handleClose}
                className="ml-4 flex-shrink-0 text-secondary hover:text-primary transition-colors"
                aria-label="Close"
            >
